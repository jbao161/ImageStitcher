...
namespace ImageStitcher
{
    internal class ImageAnimator
    {
        ...
        ///     See the ImageInfoS.cs file for the helper nested ImageInfoS class.
        ///
        ...
        /// </devdoc>
        private static List<ImageInfoS> ImageInfoSList;
        ...
        {
            if (!anyFrameDirty || image == null || ImageInfoSList == null)
            {
                return;
            }
            ...
            try
            {
                ...
                foreach (ImageInfoS ImageInfoS in ImageInfoSList)
                {
                    if (ImageInfoS.Image == image)
                    {
                        if (ImageInfoS.FrameDirty)
                        {
                            // See comment in the class header about locking the image ref.
                            lock (ImageInfoS.Image)
                            {
                                ImageInfoS.UpdateFrame();
                            }
                        }
                        foundImage = true;
                    }
        ...
        {
            if (!anyFrameDirty || ImageInfoSList == null)
            {
                return;
            }
            if (threadWriterLockWaitCount > 0)
        ...
        public static void Animate(Image image, EventHandler onFrameChangedHandler)
        {
            ...
            lock (image)
            {
                // could we avoid creating an ImageInfoS object if FrameCount == 1 ?
                ImageInfoS = new ImageInfoS(image);
            }
            ...
            try
            {
                if (ImageInfoS.Animated)
                {
                    // Construct the image array
                    //
                    if (ImageInfoSList == null)
                    {
                        ImageInfoSList = new List<ImageInfoS>();
                    }

                    // Add the new image
                    //
                    ImageInfoS.FrameChangedHandler = onFrameChangedHandler;
        ...
        public static void StopAnimate(Image image, EventHandler onFrameChangedHandler)
        {
            ...
            if (image == null || ImageInfoSList == null)
            {
                return;
            }
            ...
            try
            {
                ...
                for (int i = 0; i < ImageInfoSList.Count; i++)
                {
                    ImageInfoS ImageInfoS = ImageInfoSList[i];

                    if (image == ImageInfoS.Image)
                    {
                        if ((onFrameChangedHandler == ImageInfoS.FrameChangedHandler) || (onFrameChangedHandler != null && onFrameChangedHandler.Equals(ImageInfoS.FrameChangedHandler)))
                        {
                            ImageInfoSList.Remove(ImageInfoS);
                        }
                        break;
                    }
                }
        ...
        ///     WARNING: Also, this is the only place where ImageInfoS objects (not the contained image object) are modified,
        ...
        {
            int sleeptime = 10;
            Debug.Assert(ImageInfoSList != null, "Null images list");

            while (true)
            {
                // Acquire reader-lock to access ImageInfoSList, elemens in the list can be modified w/o needing a writer-lock.
                ...
                try
                {
                    for (int i = 0; i < ImageInfoSList.Count; i++)
                    {
                        ...
                        //
                        // sleep /10
                        int frametimerincr = Math.Max(1, (int)(sleeptime * .1));
                        ImageInfoS.FrameTimer += frametimerincr;
                        if (ImageInfoS.FrameTimer >= ImageInfoS.FrameDelay(ImageInfoS.Frame))
                        {
                            ImageInfoS.FrameTimer = 0;

                            if (ImageInfoS.Frame + 1 < ImageInfoS.FrameCount)
                            {
                                ImageInfoS.Frame++;
                            }
                            else
